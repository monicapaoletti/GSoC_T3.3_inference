...
def heun_sde_precomputed_noise(x, t, P, dW_r, dW_v, nn):
    """
    Heun SDE step using precomputed noise increments dW_r and dW_v (shape (nn,))
    """
    dt = P.dt
...
def integrate_vbjax(nn, P, B, key, record_rv, record_bold, nt, rv_decimate, bold_decimate, noise=None):
    """
    Integrate the MPR SDE + Balloon-Windkessel BOLD using vbjax.make_sde().
    This version is fully differentiable and memory efficient.
    """

    dt = P.dt
    r_period = dt * rv_decimate
    dtt = r_period / 1000.0  # BOLD time step
    t_end = P.t_end
    ...
    # --- drift and diffusion ---
    dfun = lambda x, p: f_mpr(x, 0.0, p, nn)

    def gfun(x, p):
        g_r = jnp.ones(nn) * p.sigma_r
        g_v = jnp.ones(nn) * p.sigma_v
        return jnp.concatenate([g_r, g_v])

    # --- create the differentiable SDE integrator ---
    step, sde_loop = make_sde(dt, dfun, gfun)

    # --- noise generation ---
    if noise is not None and "noise_r" in noise:
        z_t = jnp.concatenate([noise["noise_r"], noise["noise_v"]], axis=1)
    else:
        key, subkey = jax.random.split(key)
        z_t = jax.random.normal(subkey, shape=(nt, 2 * nn))

    # --- integrate neural dynamics ---
    traj = sde_loop(rv_current, z_t, P)  # shape (nt, 2*nn)

    # --- decimate for BOLD ---
    rv_d = traj[::rv_decimate]
    rv_t = jnp.arange(0, len(rv_d)) * dt * rv_decimate

    if record_bold:
        vv = jnp.zeros((len(rv_d) // bold_decimate, nn))
        qq = jnp.zeros((len(rv_d) // bold_decimate, nn))

        # integrate BOLD on decimated samples
        def bold_step(carry, i):
            s, f, ftilde, vtilde, qtilde, v, q, vv, qq = carry
            s, f, ftilde, vtilde, qtilde, v, q = do_bold_step(
                rv_d[i, :nn], s, f, ftilde, vtilde, qtilde, v, q, dtt, B
            )
            do_record_bold = (i % bold_decimate == 0)

            def save_fn(x):
                s, f, ftilde, vtilde, qtilde, v, q, vv, qq = x
                idx = i // bold_decimate
                vv = vv.at[idx].set(v)
                qq = qq.at[idx].set(q)
                return s, f, ftilde, vtilde, qtilde, v, q, vv, qq

            return jax.lax.cond(do_record_bold, save_fn, lambda x: x, (s, f, ftilde, vtilde, qtilde, v, q, vv, qq)), 0

        init_carry = (s, f, ftilde, vtilde, qtilde, v, q, vv, qq)
        final_carry, _ = jax.lax.scan(bold_step, init_carry, jnp.arange(len(rv_d)))
        s, f, ftilde, vtilde, qtilde, v, q, vv, qq = final_carry
    ...
    return {
        "rv_t": rv_t * 10,  # ms
        "rv_d": rv_d,
        "bold_t": bold_t.astype(jnp.float32) * 10,
        "bold_d": bold_d.astype(jnp.float32),
    }


def integrate(nn, P, B, key, record_rv, record_bold, nt, rv_decimate, bold_decimate, noise=None):
    """
    ...
    def loop_body(carry, i):
        ...
        # index i is the time step; noise arrays have shape (nt, nn)
        # provide deterministic noise if noise provided, otherwise fallback to zeros (not recommended)
        #noise_r = noise["noise_r"] if (noise is not None and "noise_r" in noise) else jnp.zeros((nt, nn))
        #noise_v = noise["noise_v"] if (noise is not None and "noise_v" in noise) else jnp.zeros((nt, nn))

        # pass the step index as integer i; use dynamic_slice or indexing
        #dW_r_step = P.sigma_r * noise_r[i]
        #dW_v_step = P.sigma_v * noise_v[i]

        if noise is not None:
            noise_r = noise.get("noise_r", None)
            noise_v = noise.get("noise_v", None)
        else:
            noise_r = noise_v = None

        def get_noise_step(arr, idx, sigma, nn):
            if arr is None:
                return jnp.zeros((nn,))
            noise_step = jax.lax.dynamic_index_in_dim(arr, idx, axis=0, keepdims=False)

            return sigma * noise_step

        dW_r_step = get_noise_step(noise_r, i, P.sigma_r, nn)
        dW_v_step = get_noise_step(noise_v, i, P.sigma_v, nn)

        rv_current = heun_sde_precomputed_noise(rv_current, t_current, P, dW_r_step, dW_v_step, nn)
    ...
    # Wrap the per-step function
    #loop_body_ckpt = jax.checkpoint(loop_body)

    # Scan with checkpointed loop
    #final_carry, _ = jax.lax.scan(loop_body_ckpt, init_carry, jnp.arange(nt - 1))
...
class MPR_sde:
    ...
    def run(self, par: dict = {}, x0: jnp.ndarray = None, noise: dict = None):
        P = self.P
